//! Minimal JSON value emission for transport responses.
//!
//! The artifact service links no serialization crate. Response bodies are small,
//! flat objects, so a dependency-free writer that escapes strings correctly is
//! sufficient and keeps the offline, std-only build guarantee intact.

/// Serializes an owned JSON value into a compact UTF-8 string.
