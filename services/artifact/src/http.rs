//! A dependency-free HTTP/1.1 transport for the artifact service.
//!
//! The service core deliberately links no async runtime or HTTP framework, so
...
//! [`crate::router`], writes the response, and closes the connection. Time is
//! provided by an injected clock so the routing layer stays deterministic and
//! testable; [`ArtifactServer::with_system_clock`] wires the real wall clock for
//! `main`.

use crate::{ArtifactError, ArtifactService, http_error, json::Json, router};
...
impl HttpResponse {
    ...
    pub fn error(error: &ArtifactError) -> Self {
    ...
    /// domain call (malformed method, unmatched route, bad part number).
...
/// A blocking HTTP transport bound to a running [`ArtifactService`].
#[derive(Clone)]
...
impl ArtifactServer {
    ...
    pub fn new(service: ArtifactService, clock: Clock) -> Self {
        let max_body_bytes = service.config().limits.max_multipart_bytes;
        Self { service, clock, max_body_bytes }
    }

    /// Builds a server whose clock reads the system wall clock in Unix seconds.
    pub fn with_system_clock(service: ArtifactService) -> Self {
        ...
        for stream in listener.incoming() {
            ...
            thread::spawn(move || {
                if let Err(error) = worker.serve_connection(stream) {
                    eprintln!("lawsynth-artifact: connection error: {error}");
                }
            });
        }
        Ok(())
    ...
    fn serve_connection(&self, stream: TcpStream) -> io::Result<()> {
        ...
        let response = match read_request(&mut reader, self.max_body_bytes)? {
            ...
            ReadOutcome::Request(request) => router::route(&self.service, (self.clock)(), &request),
        };
...
/// Splits a request target into its decoded-free path and raw query components.
...
mod tests {
    ...
    fn parses_request_line_headers_and_body() {
        let raw = b"POST /artifacts?x=1 HTTP/1.1\r\nContent-Type: text/plain\r\nContent-Length: 5\r\n\r\nhello";
        ...
        assert_eq!(request.path, "/artifacts");
    ...
    fn rejects_a_body_that_exceeds_the_limit() {
        let raw = b"POST /artifacts HTTP/1.1\r\nContent-Length: 10\r\n\r\n0123456789";
        let mut reader = BufReader::new(&raw[..]);
