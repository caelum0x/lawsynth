#!/usr/bin/env python3
"""Executable native-CLI conformance runner for the LawSynth runtime.

The case files are deliberately declarative. This runner builds a real
stored-ZIP ``.lsworld`` archive using the documented binary-v1 layout and then
invokes the compiled Rust CLI. It never emulates simulation or acceptance
behavior: valid worlds must round-trip through ``inspect``/``simulate`` and
every ``bad-*`` fixture must be rejected by the engine with a non-zero exit and
...
# ``tests/conformance/_conformance.py`` -> parents[1] is ``tests/`` where the
# shared engine helper lives. Insert it so the module works both under pytest
# and when a case ``run.py`` is executed standalone.
_TESTS = Path(__file__).resolve().parents[1]
if str(_TESTS) not in sys.path:
    sys.path.insert(0, str(_TESTS))

from _engine import cli  # noqa: E402

MANIFEST = b'{\n  "format": "lawsynth-world",\n  "format_version": "0.1.0",\n  "world_encoding": "lawsynth-world-binary-v1"\n}\n'

_OUTCOME_FOR_MODE = {"valid": "accepted", "invalid": "rejected", "unsupported": "unsupported"}
...
def run_case(directory: Path) -> None:
    """Execute a single conformance case against the real engine and assert."""
    ...
    assert _OUTCOME_FOR_MODE[case["case"]["mode"]] == expected["outcome"]
