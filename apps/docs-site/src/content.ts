...
const INTRODUCTION: DocumentationPageSource = {
  ...
  source: markdownDocument(
    ...
    "**Why it matters now.** As black-box ML spreads through science and engineering, LawSynth is a free, open-source (Apache-2.0) alternative that returns models you can audit: explicit differential equations instead of opaque weights, computed **deterministically and offline** so results reproduce exactly. It builds on the ideas behind symbolic regression and SINDy and packages them into one reproducible, end-to-end workflow — from raw CSV to a shareable model.",
    "## What you get",
    ...
    "LawSynth is free and open source under the **[Apache-2.0 license](https://github.com/caelum0x/lawsynth/blob/main/LICENSE)**, developed in the open on GitHub. Contributions, issues, and stars are all welcome.",
    "- **[Star LawSynth on GitHub](https://github.com/caelum0x/lawsynth)** — follow development and help others find the project.",
    "- **[Contribute](https://github.com/caelum0x/lawsynth/blob/main/CONTRIBUTING.md)** — the workspace layout, how to build and test, and the review process for pull requests.",
    "- **Get started in minutes** — install the `lawsynth` CLI, or reach for the Python and TypeScript SDKs that mirror it: [Getting started](/getting-started).",
    "## Project scope",
    "LawSynth is a Rust-first monorepo (30+ crates) with Python and TypeScript SDKs. Its capabilities span four domains, all operating on the same validated World IR and portable `.lsworld` bundle:",
    "- **Dynamics** — discover, explain, and simulate governing equations from observed data.",
    "- **Control** — controlled discovery (SINDYc), model reduction, and model-predictive control.",
    "- **Causal** — dependency hypotheses and coupling structure recovered between variables.",
    "- **Bifurcation & analysis** — stability, Lyapunov exponents, basins of attraction, invariants, and parameter sweeps.",
    "Two applied tools in the workspace build on this core engine: **GridSynth** (measured power-grid workflows) and **Information Diffusion** (cascade dynamics on networks). The forward plan — collaboration, streaming discovery, and safe community plugins — is tracked in the [roadmap](https://github.com/caelum0x/lawsynth/blob/main/ROADMAP.md), and the local, offline, deterministic core never regresses as those land.",
    "## Where to go next",
