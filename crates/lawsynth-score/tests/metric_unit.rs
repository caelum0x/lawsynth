...
fn domination_and_weighted_ranking_use_the_documented_minimization_order() {
    let simple = CandidateMetrics { mean_squared_error: 1.0, complexity: 1 };
    let worse = CandidateMetrics { mean_squared_error: 2.0, complexity: 3 };
    let accurate = CandidateMetrics { mean_squared_error: 0.5, complexity: 10 };
    ...
    assert_eq!(
        weighted_rank(
            ...
            ScoringConfig { error_weight: 1.0, complexity_weight: 0.2 },
        )
        .unwrap()
        .into_iter()
        .map(|(index, _)| index)
