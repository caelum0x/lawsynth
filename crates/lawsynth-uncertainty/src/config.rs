...
impl Default for BootstrapConfig {
    fn default() -> Self {
        Self { replicates: 1_000, seed: 0x4c_4157_5359_4e54 }
    }
}

impl BootstrapConfig {
    pub fn validate(self) -> Result<(), UncertaintyError> {
        if self.replicates == 0 { Err(UncertaintyError::InvalidBootstrapConfig) } else { Ok(()) }
    }
}
...
impl Default for PropagationConfig {
    fn default() -> Self {
        Self { draws: 10_000, seed: 0x5052_4f50_4147_4154 }
    }
}

impl PropagationConfig {
    pub fn validate(self) -> Result<(), UncertaintyError> {
        if self.draws == 0 { Err(UncertaintyError::InvalidPropagationConfig) } else { Ok(()) }
    }
}
