...
fn preserves_dataset_validation_at_the_python_boundary() {
    let mismatch = dataset_from_columns(
        vec![0.0, 1.0],
        BTreeMap::from([("x".to_owned(), vec![1.0])]),
    );
    assert!(
        mismatch
            .expect_err("column length mismatch must be rejected")
            .contains("expected 2")
    );

    let malformed_name = dataset_from_columns(
        vec![0.0],
        BTreeMap::from([("not a column".to_owned(), vec![1.0])]),
    );
